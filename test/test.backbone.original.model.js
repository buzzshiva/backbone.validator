$(document).ready(function() {
  ...
  module("Original Backbone Model Tests");
  ...
  var proxy = Backbone.Validator.extend();
  ...
  test("Model: clone", function() {
    ...
    a = new Backbone.Validator(attrs);
    b = a.clone();
    equals(a.get('foo'), 1);
    equals(a.get('bar'), 2);
    equals(a.get('baz'), 3);
  ...
  test("Model: isNew", function() {
    ...
    ok( new Backbone.Validator({          }).isNew(), "is true when there is no id");
    ok(!new Backbone.Validator({ 'id': 2  }).isNew(), "is false for a positive integer");
    ok(!new Backbone.Validator({ 'id': -5 }).isNew(), "is false for a negative integer");
  });
  ...
  test("Model: multiple unsets", function() {
    ...
    var model = new Backbone.Validator({a: 1});
  ...
  test("Model: using a non-default id attribute.", function() {
    var MongoModel = Backbone.Validator.extend({idAttribute : '_id'});
  ...
  test("Model: set an empty string", function() {
    var model = new Backbone.Validator({name : "Model"});
    model.set({name : ''});
  ...
  test("Model: clear", function() {
    var changed;
    var model = new Backbone.Validator({id: 1, name : "Model"});
  ...
  test("Model: defaults", function() {
    var Defaulted = Backbone.Validator.extend({
      defaults: {
        "one": 1,
        "two": 2
      }
    ...
    Defaulted = Backbone.Validator.extend({
      defaults: function() {
        return {
          "one": 3,
          "two": 4
  ...
  test("Model: change, hasChanged, changedAttributes, previous, previousAttributes", function() {
    var model = new Backbone.Validator({name : "Tim", age : 10});
  ...
  test("Model: change with options", function() {
    var value;
    var model = new Backbone.Validator({name: 'Rob'});
  ...
  test("Model: change after initialize", function () {
    ...
    var obj = new Backbone.Validator(attrs);
  ...
  test("Model: save within change event", function () {
    var model = new Backbone.Validator({firstName : "Taylor", lastName: "Swift"});
    model.on('change', function () {
      model.save();
  ...
  test("Model: validate after save", function() {
    var lastError, model = new Backbone.Validator();
  ...
  test("Model: validate", function() {
    var lastError;
    var model = new Backbone.Validator();
    model.validate = function(attrs) {
      if (attrs.admin) return "Can't change admin status.";
    };
  ...
  test("Model: validate on unset and clear", function() {
    var error;
    var model = new Backbone.Validator({name: "One"});
    model.validate = function(attrs) {
      if ("name" in attrs) {
        if (!attrs.name) {
          error = true;
  ...
  test("Model: Inherit class properties", function() {
    var Parent = Backbone.Validator.extend({
  ...
  test("Model: Nested change events don't clobber previous attributes", function() {
    var A = Backbone.Validator.extend({
      initialize: function() {
    ...
    var B = Backbone.Validator.extend({
      initialize: function() {
  ...
  test("Model: Multiple nested calls to set", function() {
    var counter = 0, model = new Backbone.Validator({});
    model.on('change', function() {
      counter++;
      model.set({b: 1});
      model.set({a: 1});
  ...
  test("hasChanged/set should use same comparison", function() {
    expect(2);
    var changed = 0, model = new Backbone.Validator({a: null});
    model.on('change', function() {
